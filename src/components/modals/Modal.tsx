...
export default function Modal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      {/* Backdrop com blur - EXATAMENTE igual a Gerenciar Usuários */}
      <div 
        className="fixed inset-0 bg-black/20 backdrop-blur-sm transition-opacity duration-300 ease-in-out" 
        onClick={onClose} 
      />
      
      {/* Modal com animação - EXATAMENTE igual a Gerenciar Usuários */}
      <div className="flex min-h-full items-center justify-center p-4">
        <div className={`relative w-full transform overflow-hidden rounded-2xl bg-white shadow-2xl transition-all duration-300 ease-in-out scale-100 opacity-100 ${sizeClasses[size]}`}>
          {/* Header com gradiente - EXATAMENTE igual a Gerenciar Usuários */}
          ...
          {/* Conteúdo do modal - EXATAMENTE igual a Gerenciar Usuários */}
          <div className="px-6 py-6">
            {children}
          </div>

          {/* Footer com ações - EXATAMENTE igual a Gerenciar Usuários */}
          {footer && (
